...
class SegDetector:
    ...
    def box_from_bitmap(self, pred, origin_im_size):
        ...
        for index in range(num_contours):
            ...
            if score < self.box_thresh:
                continue
            box = self.unclip(points, unclip_ratio=0.8).reshape(-1, 1, 2)
            ############ hou 2024-4-22 #############
            box_ = np.squeeze(box)
            ...
            #########################################
            box[:, 0] = np.clip(
                np.round(box[:, 0] / width * origin_im_size[1]), 0, origin_im_size[1])
            box[:, 1] = np.clip(
                np.round(box[:, 1] / height * origin_im_size[0]), 0, origin_im_size[0])
            ####################################
            box, sside = self.get_mini_boxes(box)
            box = np.array(box)
            if sside < self.min_size + 2:
    ...
    def represent(self, pred, origin_im_size):
       boxes, scores = self.box_from_bitmap(pred, origin_im_size)
       return boxes, scores
...
if __name__ == '__main__':
    ...
    img_infoes = letterbox(img,new_shape=(1728,1728),color=(125,125,125))
    im, ratio, (dw, dh) = img_infoes
    im = im - np.array([122.67891434, 116.66876762, 104.00698793])
    ...
    print(f'pred:{pred}')
    detor_info = deteor.represent(pred, (img_h, img_w))
